...
public class FinalSeg {
    ...
    private static final String PROB_EMIT = "/data/Jieba/prob_emit.txt";
    ...
    private void loadModel() {
        ...
        try {
            ...
            System.err.printf(Locale.getDefault(), "%s: load model failure!%n", PROB_EMIT);
        }
        finally {
            try {
                if (null != is)
                ...
                System.err.printf(Locale.getDefault(), "%s: close failure!%n", PROB_EMIT);
            }
        }
        System.out.printf(Locale.getDefault(), "model load finished, time elapsed %d ms.%n",
